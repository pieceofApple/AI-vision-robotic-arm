...
def is_corner_order_correct(corners):
    """
    检查角点顺序是否从左到右、从上到下
    ...
    """
    # 检查行内从左到右顺序
    row_order = all(corners[i * Nx_cor + j][0][0] <= corners[i * Nx_cor + j + 1][0][0]
                    for i in range(Ny_cor) for j in range(Nx_cor - 1))
    # 检查列内从上到下顺序
    col_order = all(corners[i * Nx_cor + j][0][1] <= corners[(i + 1) * Nx_cor + j][0][1]
                    for i in range(Ny_cor - 1) for j in range(Nx_cor))
    return row_order and col_order
...
def calibrate(cap):
    ...
    objp[:, :2] = np.mgrid[0:Nx_cor, 0:Ny_cor].T.reshape(-1, 2) * square_size
    ...
    while True:
        ...
        if ret:
            if is_corner_order_correct(corners):
    ...
    if last_valid_frame is not None:
        cv2.imwrite('calibration_finished.jpg', last_valid_frame)
        print("标定完成图已保存为 calibration_finished.jpg")
